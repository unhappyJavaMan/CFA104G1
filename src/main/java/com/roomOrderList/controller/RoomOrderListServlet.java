...
public class RoomOrderListServlet extends HttpServlet{
	...
	public void doPost(HttpServletRequest req,HttpServletResponse res) throws ServletException, IOException{
		...
		//後台修改使用
		if ("backEndgetOne_For_Update".equals(action)) {
			List<String> errorMsgs = new LinkedList<String>();
			req.setAttribute("errorMsgs", errorMsgs);
			...
			try {
				...
				/***************************3.查詢完成,準備轉交(Send the Success view)************/
				req.setAttribute("roomOrderListVO", roomOrderListVO);  // 資料庫取出的VO物件,存入req
				String url = "/back-end/roomOrderList/updateRoomOrderList.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url); //成功轉交 input.jsp
			    successView.forward(req, res);
...
//				}
				...
				/***************************2.開始修改資料*****************************************/
				RoomOrderListService roomOrderListSvc = new RoomOrderListService();
				roomOrderListVO = roomOrderListSvc.updateRoom_order_list(room_order_list_id, room_id, room_order_id, number_of_people,  special_req,room_price,service_order_id,room_type_id);
		...
		//後臺更新使用
		if("backEndUpdate".equals(action)) {
			List<String> errorMsgs = new LinkedList<String>();
			req.setAttribute("errorMsgs", errorMsgs);
			...
			try {		
				...
				/***************************2.開始修改資料*****************************************/
				RoomOrderListService roomOrderListSvc = new RoomOrderListService();
				roomOrderListVO = roomOrderListSvc.backUpdateRoom_order_list(room_order_list_id, special_req);
				...
				errorMsgs.add("修改資料失敗:"+e.getMessage());
				RequestDispatcher failureView = req.getRequestDispatcher("/back-end/roomOrderList/updateRoomOrderList.jsp");
				failureView.forward(req, res);
			}			
		}
...
//				}
				...
				/***************************2.開始新增資料***************************************/
				RoomOrderListService roomOrderListSvc = new RoomOrderListService();
				roomOrderListVO = roomOrderListSvc.addRoom_order_list(room_id, room_order_id, number_of_people,  special_req,room_price,service_order_id,room_type_id);
...
//
//		}
//>>>>>>> branch 'master' of https://github.com/Davidshyu/0216roomCFA104G1.git
		if ("getRoomOrderListByRoomOrderid".equals(action)) {
			List<String> errorMsgs = new LinkedList<String>();
			req.setAttribute("errorMsgs", errorMsgs);
			...
			try {
				/***************************1.接收請求參數****************************************/
				Integer room_order_id = new Integer(req.getParameter("room_order_id"));
				/***************************2.開始查詢資料****************************************/
				...
				/***************************3.查詢完成,準備轉交(Send the Success view)************/
				String url = "/front-end/roomOrderList/showMemRoomOrderList.jsp";
                ...
                errorMsgs.add("無法取得要修改的資料"+ e.getMessage());
                System.out.println("1111111111133333333333333");
