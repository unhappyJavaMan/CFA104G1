...
public class RoomService {
	...
	public List<RoomVO> getTypeAll(Integer room_type_id){
		return dao.getOneTypeAll(room_type_id);
	}
	
	public RoomVO checkIn(Integer room_id,String mem_id ) {
	RoomVO roomVO = new RoomVO();
		
		roomVO.setRoom_id(room_id);
		roomVO.setRoom_guest_name(mem_id);
		
		dao.checkINStatus(roomVO);

		return roomVO;
		
	}
	
	public RoomVO checkOut(Integer room_id) {
	RoomVO roomVO = new RoomVO();
		
		roomVO.setRoom_id(room_id);
		dao.checkOutStatus(roomVO);

		return roomVO;
		
	}
	public RoomVO clean(Integer room_id) {
		RoomVO roomVO = new RoomVO();
