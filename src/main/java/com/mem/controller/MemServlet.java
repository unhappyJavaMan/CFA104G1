...
//					errorMsgs.add("姓名請勿空白");
					...
					errorMsgs.put("mem_name","姓名只能是中、英文字母、數字和_ ,且長度必需在2到10之間");
...
//					errorMsgs.add("手機格式錯誤");
				...
				if (!memSvc.checkMem(mem_email)) {
					errorMsgs.put("mem_email","Email地址重複");
...
//					errorMsgs.add("請輸入電子信箱");
					errorMsgs.put("mem_email","請輸入電子信箱");
				}
				// Send the use back to the form, if there were errors
				if (!errorMsgs.isEmpty()) {
				...
				if (mem_password == null || (mem_password.trim()).length() == 0) {
//					errorMsgs.add("請輸入密碼");
					errorMsgs.put("mem_password","請輸入密碼");
				}
				// Send the use back to the form, if there were errors
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req.getRequestDispatcher("/front-end/mem/login.jsp");
				...
				if (memVO == null) {
					errorMsgs.put("mem_login","帳號或密碼錯誤，請重新輸入");
//					errorMsgs.add("帳號或密碼錯誤，請重新輸入");
				...
				if (memVO.getMem_status() == (byte)2) {
					errorMsgs.put("mem_status","此會員已被停權，請聯繫客服");
//					errorMsgs.add("帳號或密碼錯誤，請重新輸入");
				}
				// Send the use back to the form, if there were errors
				if (!errorMsgs.isEmpty()) {
...
//		        	pwdCookie.setMaxAge(0);
				...
				String url = "/front-end/mem/memindex.jsp";
...
//			List<String> errorMsgs = new LinkedList<String>();
			...
			try {
				...
				String mem_password = req.getParameter("mem_password").trim();
				if (mem_password == null || mem_password.trim().length() == 0) {
					errorMsgs.put("mem_password","舊密碼請勿空白");
//					errorMsgs.add("原本密碼請勿空白");
				...
				if (mem_password_new == null || mem_password_new.trim().length() == 0) {
					errorMsgs.put("newmem_password","新密碼請勿空白");
...
//					errorMsgs.add("密碼重複請勿空白");
					...
					errorMsgs.put("remem_password","新密碼與密碼重複不同");
//					errorMsgs.add("新密碼與密碼重複不同");
				...
				if (memVO == null) {
					errorMsgs.put("mem_password","舊密碼錯誤，請重新輸入");
...
//				mailService.sendMail(mem_email, subject, messageText);
		...
		if ("reverify".equals(action)) { // 來自signUp.jsp的請求
