...
<%
	NewsListService newsListSvc = new NewsListService();
 	List<NewsListVO> list = newsListSvc.getAll();
					...
					<div class="card">
						...
						<div class="card-body">
							<table class="table table-striped" id="table1">
								...
								<tbody>
									<c:forEach var="newsListVO" items="${list}" varStatus="s">
										<tr>
											<td>${newsListVO.newsId}</td>
											<td>${newsListVO.title}</td>
											<td>${newsListVO.status}</td>
											<td>${newsListVO.newsComeFrom}</td>
...
<%-- 											<td>${productVO.piSta}</td> --%>
											...
											<td>
												<form
													...
													<input type="hidden" name="newsId" value="${newsListVO.newsId}">
										...
										<script>
        	 								...
        	 								document.open("<%=request.getContextPath()%>/newsList/NewsList.do?newsId=${newsListVO.newsId}&action=getOneForDiplay", "" ,"height=550,width=850,left=65,top=100,resizable=yes,scrollbars=yes");
         									}
         									function presses_img${s.count}(){
            	 							document.open("<%=request.getContextPath()%>/newsListImage/NewsListImage.do?newsID=${newsListVO.newsId}&action=showImages", "" ,"height=550,width=850,left=65,top=100,resizable=yes,scrollbars=yes");
             								}
        								</script>
									</c:forEach>
								</tbody>
